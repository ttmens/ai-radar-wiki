...
DIGEST_PATH = digest_files[-1] if digest_files else None
DATE = os.path.basename(DIGEST_PATH).replace(".md", "") if DIGEST_PATH else "unknown"
...
def main():
    if not APP_SECRET or "..." in APP_SECRET:
        print("❌ FEISHU_APP_SECRET is missing or truncated in .env file")
        print("Please update ~/.hermes/.env with the full secret value.")
        return

    print(f"Step 1: Getting tenant token... (APP_ID={APP_ID}, SECRET len={len(APP_SECRET)})")
    tenant_token = get_tenant_token()
    if not tenant_token:
        print("❌ Failed to get tenant token")
        return
    
    print(f"Step 2: Building card markdown from {DIGEST_PATH}...")
