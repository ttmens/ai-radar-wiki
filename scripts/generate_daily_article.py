...
def load_data():
    ...
    Fallback logic: if today has no items yet (explorer hasn't run),
    use the most recent day with data so the article can still generate.
    """
    ...
    if os.path.exists(GRAPH_JSON):
        ...
        # Group non-concept nodes by date
        by_date = {}
        for node in graph.get("nodes", []):
            d = node.get("date", "")[:10]
            if not d or node.get("type") == "concept":
                continue
            by_date.setdefault(d, []).append(node)
        # Prefer today; otherwise pick the latest date with data
        if by_date.get(today):
            ...
            if available:
                chosen_date = available[-1]
                items = by_date[chosen_date]
                print(f"⚠️ 今日({today})无数据，fallback 到 {chosen_date}（{len(items)} 条）")

    items.sort(key=lambda x: x.get("pm_score", 0), reverse=True)
    return daily, items, chosen_date
    ...
    for attempt in range(1, max_retries + 1):
        try:
            with httpx.Client(timeout=30) as client:
                ...
                if not token:
                    print(f"⚠️ Failed to get Feishu token (attempt {attempt}):", resp.text[:200])
                    if attempt < max_retries:
                        import time; time.sleep(5 * attempt)
                        continue
                    return

                # 2. Upload File — ⚠️ MUST use file_type=stream for HTML files
                # file_type=file rejects HTML with error 234001
                ...
                with open(filepath, "rb") as f:
                    ...
                    if not file_key:
                        print(f"⚠️ Failed to upload file (attempt {attempt}):", resp.json())
                        if attempt < max_retries:
                            import time; time.sleep(5 * attempt)
                            continue
                        return
                ...
                # 3. Send Message
                chat_id = "oc_a2aedb3e0b69d55d2c73a83c69427f2e"
                ...
                if resp.json().get("code") == 0:
                    print(f"🚀 文件已发送到聊天窗口: {safe_filename}")
                    return
                else:
                    print(f"⚠️ 发送消息失败 (attempt {attempt}):", resp.json())
                    if attempt < max_retries:
                        import time; time.sleep(5 * attempt)
                        continue
                    return
        except Exception as e:
            print(f"⚠️ Send error (attempt {attempt}): {e}")
            if attempt < max_retries:
                import time; time.sleep(5 * attempt)
                continue
            return
...
def main():
    ...
    # 1. Load Data
    daily, items, chosen_date = load_data()
    date = chosen_date or daily.get("date", datetime.now(BJ_TZ).strftime("%Y-%m-%d"))
    overview = daily.get("overview", "AI Daily") or "AI Daily"
    ...
    os.chdir(WIKI_DIR)
    import subprocess
    safe_overview = overview[:50].replace("'", "").replace('"', '')
    subprocess.run(["git", "add", filepath], check=False)
    subprocess.run(["git", "commit", "-m", f"daily-article: {date} {safe_overview}"], check=False)
    ...
    subprocess.run(["python3", "build_articles_page.py"], capture_output=True, check=False)
    os.chdir(WIKI_DIR)
    subprocess.run(["git", "add", "articles.html", "articles_template.html", "daily-articles/"], check=False)
    subprocess.run(["git", "commit", "-m", f"articles: add {date} + rebuild archive"], check=False)
    subprocess.run(["git", "push"], capture_output=True, check=False)
    print("✅ Archive rebuilt and pushed")

    # 5. Publish to WeChat Official Accounts
    print("📱 Publishing to WeChat Official Accounts...")
    os.chdir(os.path.expanduser("~/.hermes/scripts"))
    title = sanitize_filename(overview)
    # Corp account: capture output for debugging
    corp_output = os.popen(f"python3 publish_to_wechat.py --account corp --html-file {filepath} --title '{title}' 2>&1").read()
    print(corp_output)
    # Personal account: draft only
    personal_output = os.popen(f"python3 publish_to_wechat.py --account personal --html-file {filepath} --title '{title}' 2>&1").read()
    print(personal_output)
    os.chdir(WIKI_DIR)
