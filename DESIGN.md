...
- **内容顺序** (从上到下):
  ...
  4. **日/周视图切换**: 透明 tab 风格分段控件，固定在面板顶部
- **折叠**: 点击 `✕` 按钮折叠面板，左侧边缘出现竖向展开按钮 `📅 情报摘要`
- **展开按钮**: `position: absolute; left: 0; top: 50%`，竖向排版，隐藏时显示

#### 日/周视图切换（v3.9.1 新增）
- **样式**: 透明背景，tab 风格分段控件，Linear/Notion 美学
- **位置**: 固定在面板顶部，零边距紧贴"今日情报"标题下方
- **功能**:
  - 日视图：显示当日节点列表 + 叙事主线
  - 周视图：显示支柱趋势状态、叙事链（去重后）、矛盾信号
  - 周视图标题：`📈 本周情报趋势 (第 X 周)` + `✕` 关闭按钮
- **事件绑定**：DOM 渲染完成后自动绑定点击事件，确保 100% 响应
...
- **版本号**: Header 显示版本标签（如 `v3.10.0`），用于缓存验证
