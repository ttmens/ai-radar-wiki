...
每日情报 Digest (`daily-digest/`) 从 `graph.json` 读取**当天全部节点**，按 `pm_score` 排序后生成 Markdown 摘要。不再依赖独立数据文件。
...
```
┌─────────────────────────────────────────────────────────────────┐
│ [🧠 AI Radar]        [📅 收起/展开] [📡 RSS] [nodes] [edges]    │
├─────────────────────┬─────────────────────────┬───────────────┤
│                     │                         │               │
│ 📅 今日情报摘要  [✕]│                         │ [✕] 详情面板   │
│ ─────────────────── │                         │               │
...
│   🔧 工具生态 PM 0.63│                         │               │
│ ▌ microsoft/...     │                         │ 📝 摘要总结   │
│   🔧 工具生态 PM 0.61│    交互式知识图谱         │ (120字精炼)   │
│                     │    (vis-network.js)       │               │
│ ▌ meilisearch/...   │    - 力导向图             │ 📄 原文预览   │
│   📱 产品模式 PM 0.61│    - 可拖拽/缩放           │ [原文预览框]   │
│                     │    - 节点颜色=分类        │               │
│ ... (Top 10)        │                         │ [🔗查看原文]   │
│                     │                         │ [📋完整原文]   │
│                     │                         │               │
...
#### 左侧仪表盘 (Dashboard)
- **默认状态**: 展开，`top: 64px; left: 20px`
- **宽度**: 320px
- **最大高度**: `calc(100vh - 80px)`，超出滚动
- **内容顺序** (从上到下):
  1. **今日情报摘要 (Top 10)**: 按 PM Score 排序的今日 Top 10 项目
     - 每条含：名称 + 分类徽章 + PM Score + 摘要（最多 100 字）
     - 点击自动聚焦到图谱对应节点
  2. **面板内收起按钮**: `收起` 按钮可折叠面板内容
- **折叠**: 点击 Header 的 `📅 收起/展开` 按钮

#### 图例 (Legend)
- **位置**: 画布左下角固定 `bottom: 20px; left: 20px`
- **面板收起时**: 跟随滑动 `transition: transform 0.3s ease, opacity 0.3s ease`
- **内容**: 四大分类的颜色标识 + 中文说明
- **样式**: 半透明背景 `rgba(22,27,34,0.95)`，圆角，毛玻璃效果
...
| v3.6.0 | 2026-05-08 | Schema 标准化：统一实际字段 id/label/pillar/summary/date/pm_score/url/raw_content/type/tags/source_type，移除过时字段 group/summary_cn/pm_relevance/source_url/keywords；新增 3.5 日报 Digest 数据源说明；采集频率更新为每 4 小时
