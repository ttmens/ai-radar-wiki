...
│  摘要面板  │                            │  (右侧面板) │
│            │                            │           │
│  · Headline│     交互式知识图谱           │  点击节点 │
│  · Insights│     (vis-network.js)        │  显示详情 │
│  · Watch   │     - 力导向图              │           │
│  · Stats   │     - 可拖拽/缩放           │           │
│            │                            │           │
├────────────┴────────────────────────────┴───────────┤
...
#### 左侧仪表盘 (Dashboard)
- **默认状态**: 展开
- **内容**: LLM 生成的结构化每日摘要（非文章堆砌）
- **结构**:
  - **Headline**: 一句话总结今日最重要的趋势（20字以内）
  - **Insights**: 2-3 个关键洞察/趋势，可展开/收起
    - 每个洞察含：标题 + 2-3 句话说明 + 2-3 篇佐证文章链接
    - 点击文章链接自动聚焦到图谱对应节点
  - **Watch List**: 值得关注的单个项目/论文/事件
  - **Stats**: 一句话数据概览
- **折叠**: 点击 Header 按钮或面板内按钮切换显示/隐藏

#### 图例 (Legend)
- **位置**: 画布上（非固定），跟随面板滑动
- **面板展开时**: left = 340px（在面板右侧）
- **面板收起时**: left = 20px（滑动画布左下角）
...
- **内容**:
  ...
  - PM Score (0-10)
  - 📝 摘要总结：精炼版摘要（前 150 字）
  - 📄 原文预览：来源域名 + 类型 + 日期 + 完整摘要
  - 🔗 查看原文按钮 (跳转 source_url)
  - 📋 完整原文按钮（切换显示纯摘要 vs 带元数据的预览）
